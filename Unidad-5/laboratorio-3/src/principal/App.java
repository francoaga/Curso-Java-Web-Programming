...
public class App {
	...
	public static void main(String[] args) {
		...
		loadAplicacion(inscripcion, gestor, input);
		input.close();
	}
	
	private static void loadAplicacion(SistemaInscripcion inscripcion, GestorCursos gestor,  Scanner input) {
		altaCursosPrueba(gestor);
		...
		while(seguir) {
			...
			try {
				...
				opcionesMenu(opcion, input, inscripcion, gestor);
	...
	private static void opcionesMenu(int opcion, Scanner input, SistemaInscripcion inscripcion, GestorCursos gestor) {
		limpiarBuffer(input);
		
		switch(opcion) {
			case 1:
				agregarAlumno(inscripcion, gestor, input);
			break;
			case 2:
				agregarProfesor(inscripcion, gestor, input);
			break;
			case 3:
	...
	private static void agregarProfesor(SistemaInscripcion inscripcion, GestorCursos gestor, Scanner input) {
		...
		while(seguir) {
			try {
				...
				cargarCursos(gestor, profesor, input);
	...
	private static void agregarAlumno(SistemaInscripcion inscripcion, GestorCursos gestor, Scanner input) {
		...
		while(seguir) {
			try {
				...
				cargarCursos(gestor, alumno, input);
	...
	private static void cargarCursos(GestorCursos gestor, Persona p, Scanner input) {
		System.out.println("\n--CARGA DE CURSOS--");
		consultarCantidadDeCursos(input);
		
		for(int i = 1; i <= parametros.getCantCursosInscripto(); i++) {
			
			mostrarCursosHabilitados(gestor);
			System.out.println("Curso " + (i));
			System.out.print("Id curso: ");
			Curso c = gestor.getCurso(input.nextInt());
			
			if(p instanceof Profesor) {
				((Profesor)p).agregarCurso(c.getId());
			}else {
				((Alumno)p).agregarCurso(c.getId());
			}
		}
	}
	
	private static void mostrarCursosHabilitados(GestorCursos gestor) {
	...
	private static void altaCursosPrueba(GestorCursos gestor) {
		String[] cursos = {"Java Web Programming 17", "Desarrollo Web con HTML", "SQL Server Programming 2019",
				"Javascript desde cero", "Introduccion a Python", "Git: Desarrollo Coloborativo",
				"Introduccion a Bases de Datos y SQL", "Patrones de Diseño"};
